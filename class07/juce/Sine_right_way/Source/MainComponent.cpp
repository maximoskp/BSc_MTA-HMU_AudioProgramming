...
void MainComponent::getNextAudioBlock (const juce::AudioSourceChannelInfo& bufferToFill)
{
    ...
    if (sample_rate > 0.0) { // make sure that prepareToPlay has been called
        ...
        for (auto i = 0; i < bufferToFill.numSamples; i++) {
            ...
            // time_fraction = (float)total_samples / sample_rate;
            leftBuffer[i] = amplitude * std::sin( current_angle );
            rightBuffer[i] = amplitude * std::sin( current_angle );
            // total_samples++;
...
// __added__ vvv
// implement functions (declared in .h) here
...
void MainComponent::changeFreq() {
    ...
    juce::Logger::writeToLog("frequency: " + juce::String(frequency));
}
// __added__ ^^^
